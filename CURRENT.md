# CURRENT — Cross-chat recovery authority

**Repository name:** `formula` is temporary only; it is not the final product name.

Recover repository evidence before reasoning. Do not reconstruct implementation state from chat memory when these files are available.

## Primary authorities

1. [`docs/design/README.md`](docs/design/README.md) — frozen D1-D5 design precedence.
2. [`docs/roadmap/2026-08-28-implementation-roadmap.md`](docs/roadmap/2026-08-28-implementation-roadmap.md) — frozen roadmap P0 onward.
3. [`docs/checkpoints/2026-09-06-p11-federation-breadth.md`](docs/checkpoints/2026-09-06-p11-federation-breadth.md) — exact finally frozen P11 recovery authority.
4. [`docs/superpowers/specs/2026-09-06-p11-federation-breadth-design.md`](docs/superpowers/specs/2026-09-06-p11-federation-breadth-design.md) — frozen P11 design.
5. [`docs/superpowers/plans/2026-09-06-p11-federation-breadth.md`](docs/superpowers/plans/2026-09-06-p11-federation-breadth.md) — executed P11 implementation/freeze plan.
6. [`docs/checkpoints/2026-09-05-p10-self-expansion-hardening.md`](docs/checkpoints/2026-09-05-p10-self-expansion-hardening.md) — exact frozen P10 predecessor authority.
7. [`docs/design/2026-08-28-d1a-self-hosting-bootstrap-amendment.md`](docs/design/2026-08-28-d1a-self-hosting-bootstrap-amendment.md) — frozen P12 bootstrap/trust-reduction design authority.

## Exact current implementation state

**P11 — Federation Breadth: FINALLY FROZEN.**

Canonical branch:

```text
implementation/p11-federation-breadth
```

Exact finally frozen P10 predecessor:

```text
3aeb61daf4d575db0f018245ee271597ad475e7b
workflow: P10 canonical proof
run/job:  34024846890 / 101463880804
result:   success
```

P11 source proof:

```text
b5377eb78c7540d927fca1aea9e04ca5b3a56371
workflow: P11 canonical proof
run/job:  34030804901 / 101479821966
result:   success
```

P11 exact finally frozen documentation-bearing proof boundary:

```text
6f8ce7bb6702ea1baf119aab9950aa5ba0f87283
workflow: P11 canonical proof
run/job:  34031806639 / 101482593250
result:   success
```

Permanent P11 workflow identity:

```text
.github/workflows/p11-canonical-proof.yml
blob SHA: 97192299b4ea2aac469da38b4885e2608b2d7bd3
permissions: contents: read
Rust: 1.98.0
host: x86_64-unknown-linux-gnu
```

The source-to-docs delta was exactly two recovery files: this file and the P11 checkpoint. The unchanged canonical workflow passed on the exact docs head. Stable federation identities reproduced exactly; only the expected source-bound `P11_SOURCE_SHA` and `P11_MANIFEST` changed.

## Frozen P11 identities

Final docs-run evidence:

```text
P11_SOURCE_SHA=6f8ce7bb6702ea1baf119aab9950aa5ba0f87283
P11_P10_PREDECESSOR=sha256:37a0309762564ecdec48792b951c1d0eaf6fb5342c354cdd7d58f57934079486
P11_MANIFEST=sha256:2d503196f12098786b730c200f4032f1e3c84ebea0221c4f428fd3d6532eb1b3
P11_SAT_PACKAGE=sha256:c3351c1d8e030cf01d29c70d1d7f3119e578b97dcd20de013b7f780d37daede2
P11_SAT_ADAPTER=sha256:3d41a245fd2b28d0387b104e3efc0ed7bbd31a2582d3debead36cc2817aa4862
P11_ARITHMETIC_PACKAGE=sha256:adbabc997de811b254ad9e7052324d6a89d57014024c3c07b0fc9e1391f35e8d
P11_ARITHMETIC_ADAPTER=sha256:7fca1ad7b7b79a9af6de140769008e20d86df1036c8c584e745a1ea9ccb793b5
P11_BRIDGE=sha256:7ffc888a9ee6d9703ac6e52b61dc1f1fb086d974cd5af7981fa9d84e27266406
P11_NEGATIVE_CONTROLS=sha256:3faadf4580899f87e2a6c8a9a26e108b8d40121981f5a32b6de40f5700a40375
P11_FINAL_TARGET=sha256:dd35426c857ad922d8439e7d539fabe34d2daabb9f255dd35953b57a752a8844
```

Frozen marker contract:

```text
PASS P11_SAT_LRAT_CHECKED
PASS P11_EXACT_ARITHMETIC_CHECKED
PASS P11_FEDERATION_PROVENANCE_BOUND
PASS P11_SHARED_FACT_POLARITY_PRESERVED
PASS P11_BRIDGE_CONTRACT_ENFORCED
PASS P11_HETEROGENEOUS_COOPERATION
PASS P11_PRODUCER_IDENTITY_UNTRUSTED
PASS P11_NEGATIVE_CONTROLS
PASS FEDERATION_BREADTH_PROVED
```

## Authority boundary preserved

```text
external producer -> untrusted candidate/evidence
independent checker -> evidence authority
adapter -> route/translation contract only
CertifiedFederationFact -> checked provenance binding
BridgeContract -> explicit directional cross-package contract
SharedFact -> world/subject/polarity/evidence-bound exchange
safe composition -> required for cooperation
independent P11 verifier -> final manifest replay authority
```

Package, producer, adapter, bridge existence, successful execution or network reachability never creates mathematical authority.

## P0-P11 authority chain

```text
P0 reproducible repository/build + architecture firewall
P1 deterministic identity + immutable atomic authority store
P2 independent checker/certificate authority
P3 theory packages + capability closure
P4 deterministic query/compiler/campaign planning
P5 bounded CandidateSpace + discovery
P6 sealed First-Light harness + blindness gates
P7 checker-authorized atomic promotion into immutable U1
P8 independently authorized native realization + exact dispatch
P9 canonical First-Light closure + zero-rediscovery reuse
P10 self-expansion hardening + explicit grammar/proof/realization evolution authority
P11 heterogeneous independently checked federation + explicit safe Shared Fact bridges
```

## Next actionable roadmap boundary

**P12 — Self-hosting Bootstrap / Trust Reduction.**

P12 must begin with mandatory targeted research spike `RS-BOOTSTRAP-SEED`, comparing CakeML, Mes/stage0, verified-compilation and diverse-compilation donor patterns against the actual post-P11 codebase. The selected bootstrap strategy must preserve:

```text
successor generator != successor validator
successor cannot self-authorize
self-hosting cannot certify/promote mathematics
bootstrap provenance is first-class
bootstrap generation T_g is distinct from universe generation U_g
```

The P12 gate is a selected authority-critical component rebuilt by the self-host path and accepted only after an independent path validates the successor.

## Non-recursive freeze rule

`6f8ce7bb6702ea1baf119aab9950aa5ba0f87283` is the exact P11 proof boundary. Later documentation-only commits that record this already-earned result do not redefine the boundary.

## Freeze state

```text
P11 source proof:          PROVED
P11 docs proof:            PROVED
frozen proof head:         6f8ce7bb6702ea1baf119aab9950aa5ba0f87283
source run/job:            34030804901 / 101479821966
docs run/job:              34031806639 / 101482593250
canonical workflow blob:   97192299b4ea2aac469da38b4885e2608b2d7bd3
P11 final freeze:          FINALLY FROZEN
FEDERATION_BREADTH_PROVED: FINAL RECOVERY AUTHORITY
next roadmap phase:        P12 — self-hosting bootstrap / trust reduction
```
