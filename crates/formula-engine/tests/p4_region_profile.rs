...
fn region_rejects_context_mismatch() {
    ...
    let wrong_world =
        CompilerAuthoritySnapshot::new(d(1), d(98), packages(), vec![], vec![], vec![]);
    assert_eq!(
...
fn theory_profile_keeps_certified_facts_separate_from_estimates() {
    ...
    let profile = TheoryProfile::compile(
        &region,
        &[
            ProfileFact::new("finite", d(60)),
            ProfileFact::new("exact", d(61)),
        ],
        &[
    ...
    let estimates_only =
        TheoryProfile::compile(&region, &[], &[OperationalEstimate::new("finite", 1000)]);
