...
P10 starts from that exact finally frozen P9 boundary and does not rewrite P9 proof authority.

## Canonical P10 source proof
...
The one-clean-state integration is:

```text
crates/formula-first-light/tests/p10_self_expansion_hardening.rs
```

It executes this authority chain rather than merely comparing a marker list:

```text
frozen P9 predecessor
 -> total promotion-class registry
 -> U_g with existing Integer/Rational packages
 ...
 -> realization rollback without deleting newer history
 -> NC10-01...NC10-12 concrete rejection paths
 -> canonical SelfExpansionProofManifest
...
The clean-state proof executes each concrete failure/rejection path before constructing the exact-once negative-control manifest:
...
These controls prove that class policy cannot be bypassed, witness evidence remains admission/authority-bound, automatic nogoods require scope, route activation requires preservation evidence, CandidateSpace cannot cross grammar generations silently, automatic metaprimitives require the strict gate, non-conservative change cannot inherit proof authority silently, proof evolution remains checker-authorized, realization-only upgrades cannot smuggle semantic admission, and rollback cannot rewrite immutable history.
...
independent P10 verifier
frozen P9 predecessor canonical proof
predecessor architecture firewalls
formula-core/check/store/realize/packages/engine/first-light tests
workspace tests
workspace build
rustfmt --check
Clippy -D warnings
...
rollback -> active-selection change only; immutable generations/variants remain replayable
...
1. Promotion classes have one deterministic policy registry with class-specific activation/effect constraints.
2. Newly admitted evidence can unlock capability already defined by an unchanged theory package.
3. Scoped nogoods and preservation-gated routes can affect later grammar/search state without becoming authority sources.
4. Grammar is generation-addressed and CandidateSpace identity is bound to the exact grammar generation.
5. Metaprimitive/search-method promotion may remain shadow-only; default automation requires a stricter checker gate.
6. Semantic change has explicit freshness outcomes; non-conservative change cannot silently inherit proof authority.
7. Proof transport/repair creates new structural evidence only under exact checker authorization.
...
9. Supersession and rollback preserve immutable authority and realization history.
10. A canonical replay manifest plus independent verifier binds complete P10 evidence and negative controls.
...
automatic metaprimitive activation without the strict gate
